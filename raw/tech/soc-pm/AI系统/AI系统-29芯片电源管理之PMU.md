...
source: "https://zhuanlan.zhihu.com/p/2021882005911028169"
author:
  - "[[thatway程序员]]"
published:
created: 2026-05-03
description: "电源管理是SoC中比较重要的一个部分，特别是在bringup阶段，芯片的供电、复位、时钟等如果配置不对，系统就会启动失败，并且在流片失败的例子中电源管理会占一大部分，如果出问题对系统是致命的。在AI SoC中，耗电…"
tags:
  - "clippings"
---
...
![](https://pic1.zhimg.com/v2-d1f33d69fa1970e56aed33b472b8610c_1440w.jpg)

**电源管理** 是SoC中比较重要的一个部分，特别是在 **bringup** 阶段，芯片的 **供电、复位、时钟等** 如果配置不对，系统就会启动失败，并且在 **流片失败** 的例子中电源管理会占一大部分，如果出问题对系统是 **致命** 的。

在AI SoC中，耗电量最大的就是NPU，需要针对其使用独立的电源芯片供电，保障稳定性。另外对于执行AI功能的芯片对功耗要求较高，特别的费电，电力成本甚至比芯片贵很多。

**[PMU](https://zhida.zhihu.com/search?content_id=272214618&content_type=Article&match_order=1&q=PMU&zhida_source=entity)** （Power Management Unit）有两种实现方法， **一种就是传统的基础在A核附属的一个硬件模块，另外一种就是独立核和固件的SCP** ，在大规模AI SoC中一般采用的是 **独立固件** 的控制。其区别如下表所示：

![](https://pica.zhimg.com/v2-dda032780fbb8cc60cf72f67f7423212_1440w.jpg)

独立固件的实现就是SCP，之前关于SCP介绍过几篇文章：

[ARM SCP入门-简介和代码下载编译](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzUzMDMwNTg2Nw%3D%3D%26mid%3D2247484440%26idx%3D1%26sn%3De08b89ca8526eb6ad19ed2f55c61b80e%26scene%3D21%23wechat_redirect)

[ARM SCP入门-framework框架代码分析](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzUzMDMwNTg2Nw%3D%3D%26mid%3D2247484453%26idx%3D1%26sn%3D2ffa12dee95867544bb54b430d6cd5f4%26scene%3D21%23wechat_redirect)

[ARM SCP入门-AP与SCP通信](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzUzMDMwNTg2Nw%3D%3D%26mid%3D2247484475%26idx%3D1%26sn%3D09183060a3506b1fdcd0af4d4877c89f%26scene%3D21%23wechat_redirect)
...
**PMU** （电源管理芯片）是一种高度集成化的电源管理方案，它集成多路的 **[LDO](https://zhida.zhihu.com/search?content_id=272214618&content_type=Article&match_order=1&q=LDO&zhida_source=entity) 和 [DC-DC](https://zhida.zhihu.com/search?content_id=272214618&content_type=Article&match_order=1&q=DC-DC&zhida_source=entity) ，以及相应的检测和控制电路，其核心结构通常是PWM控制器和MOSFET** 。高集成度的PMU器件可以有效减小电路板占用面积和器件数量，因此特别适合应用在便携电子产品中。而且随着PMU对电源组控制的 **细分程度越高** ，PMU对系统各模块的 **供电控制更加精准** ，能够有效地降低电路系统的整体功耗。在现代的SoC中更是集成了PMU。其组成如下：

1. Buck结构 **DC-DC** 电源（降压型稳压器）；
2. **LDO** 低压差线性稳压电源；
3. **控制电路** 和过压、欠压、过温等有额外的保护电路；
4. **电流检测** 和电池 **管理** ；
5. 电路所需要的 **电阻、小容量电容、电感** 等。
...
![](https://picx.zhimg.com/v2-c05af6b45b01831ca15fc36cc669d89b_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-0769cd0264d1d811ce32c82964685e57_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-d6e39039ba1b9b2e6abb793d124e3ceb_1440w.jpg)
...
举例使用： **[RT1052](https://zhida.zhihu.com/search?content_id=272214618&content_type=Article&match_order=1&q=RT1052&zhida_source=entity) 芯片电源管理系统** 的第一个组成部分，称为集成PMU（集成电源管理单元）。是为了简化外部电源接口而设计的。它由一组辅助电源组成，可通过两个或三个主电源实现SoC操作。使用集成PMU的电源树的高级框图如图所示。

![](https://picx.zhimg.com/v2-4b6753fc6b205d8ab46f2a2a11760641_1440w.jpg)

电源管理单元（PMU）旨在 **简化外部电源接口** 。电源系统可分为输入电源及其特性，集成电源变换和控制元件，以及最终负载互连和要求。典型的电源系统使用PMU如下图所示。

![](https://pic1.zhimg.com/v2-11257407a5e48c5e1bbc0daddd578108_1440w.jpg)
...
| 运行模式 | 说明 |
| --- | --- |
| 超载运行(Overdrive Run) | •CPU运行速度为600 MHz，过驱动电压为1.275 V.•全速总线频率•启用所有外设并以目标频率运行•所有PLL均已启用 |
| 满载运行(Full-Speed Run) | •CPU运行速度为528 MHz，满载，低电压为1.15 V. •全速总线频率 •启用所有外设并以目标频率运行 •所有PLL均已启用 |
| 低速运行(Low-Speed Run) | •CPU运行频率为132 MHz，电压降至1.15 V. •半速内部总线频率 •某些PLL断电 •20％外设处于活动状态，其他外设处于低功耗模式 |
| 低功耗运行(Low-Power Run) | •CPU以24 MHz运行，较低电压为0.95 V. •内部总线频率为12 MHz •所有PLL均断电，OSC24M断电，RCOSC24使能 •高速外围设备断电 |

![](https://pic4.zhimg.com/v2-a300fdc02d5415f86207a469f6d53aa5_1440w.jpg)

参考：

[doc.embedfire.com/mcu/i](https://link.zhihu.com/?target=https%3A//doc.embedfire.com/mcu/i.mxrt/i.mxrt1052/zh/latest/doc/chapter38/chapter38.html)
...
![](https://pic3.zhimg.com/v2-975d18066a00f6c154bef1b69ca33146_1440w.jpg)
...
1. [sohu.com/a/770027860\_12](https://link.zhihu.com/?target=https%3A//www.sohu.com/a/770027860_121097735)
...
- 电源域 **上下电** ，重置控制
- A55CPU电源管理，包括：
- **CPU核心电源模式的转换** ，如ON、OFF、OFF\_EMU、DB\_RECOV等
- **集群电源模式转换** ，如FULL\_ON 和OFF
- 整个CPU子系统 **断电并中断唤醒**
- 芯片 **正常和深度睡眠** 的进入和退出
- 芯片超深睡眠的退出
- 分级依赖功率域的安全功率控制机制
...
![](https://pic1.zhimg.com/v2-dac5ef9241a60d0b5def494d8c69a6ec_1440w.jpg)
...
1. IdleReq 0->1.
2. 向启动器niu传播请求。
3. IdleAck0->1（表示成功分发到启动器niu）。
4. 在启动器中，SlvRdy 1->0。
5. 插座箱1->在所有转会完成后的0。
6. 启动器向电源控制器报告空闲状态0->1。
7. 向目标niu传播空闲请求。
8. 在目标上，在所有转移完成后的插座1->0（独立于SlvRdy）。
9. 目标目标向电源控制器报告。
10. 电源控制器报告怠速0->1（已完成）。
...
1. Run：就是运行态
2. IDLE：没有线程运行。CPU进入WFI，等待中断。外设都还保持活动。
3. Normal deep sleep：CPU和所有PLL关闭。高速外设进入门控，低速外设以低频率运行，DRAM进入自刷新，子系统断电。
4. Ultra deep sleep：除VDD\_AON外都断电。RTC/PMU/STCU/EFUSE/OSC\_TRIM/WAKEUPIO可以工作。
...
- STCU触发：PD\_BIGAON, PD\_PERI,PD\_CPUTOP,PD\_CPU\_C0
- CPU A55触发：其他的功率域和LBIST，MBIST测试。

基于软件的LBIST，MBIST测试，必须在虚拟断电模式下运行。数据流图如下：

![](https://pic2.zhimg.com/v2-d2421632ff8ccef066541bc95e20df81_1440w.jpg)
...
> 什么是 **PCSA** ？  
> 随着SOC的复杂性增加，例如 **多种异构核** 上又运行了不同的OS，不同的子系统间（例如 **Linux、ISP、NPU、FSI、eSecur** e等）相互独立，为了满足功耗的控制管理，这些子系统间需要进行协调，难度增大。为了更好的功耗管理，需要从系统中其他的控制器和应用处理器中抽象出来各种电源或其他系统管理任务，进行 **集中管理** ，利用一个独立的控制器核心实现。因此ARM提出了 **功耗控制系统架构** （power control system architecture，简称 **PCSA** ），用来规范芯片功耗控制的逻辑实现。

注：PCSA参考ARM规范文档

《DEN0050D\_Power\_Control\_System\_Architecture.pdf》可以去ARM官网下载。
...
![](https://pic1.zhimg.com/v2-2bda55010f623f2c6ba08623ff19227a_1440w.jpg)
...
1. 计时器事件：SCP有本地计时器资源，可用于触发系统唤醒和任何周期性动作，如监控。
	2. 唤醒事件：响应唤醒请求，包括由路由到断电核心的中断引起的GIC唤醒请求，以及来自其他代理的系统访问请求。
	3. 调试访问电源控制：响应来自调试访问端口的请求和相关控件的请求，包括调试基础设施的电源管理。
	4. 看门狗事件和系统恢复操作：在本地看门狗超时时，SCP可以执行一个重置和重新初始化序列。
5. **系统感知功能** ：
1. SCP可以协调来自OSPM和其他代理对共享资源的请求。例如，它可以控制到主存的路径，或进入SoC睡眠模式和退出，而不需要AP核心活动。
	2. SCP可负责监测传感器和测量功能。监控任务可能包括过程和温度传感器的数据收集和相关的操作，如操作点优化和报警条件。
	3. SCP在操作点选择中的作用可以扩展到必要时覆盖OSPM方向，以确保系统的电气和热保护。
![](https://pic4.zhimg.com/v2-ef90f067ad7c0b57fe074b52200504ff_1440w.jpg)

![](https://picx.zhimg.com/v2-7113355d49c9a74d72bb708c4a8dec5f_1440w.jpg)

SCP的硬件实现举例：

![](https://picx.zhimg.com/v2-0e4a6def93bde3049b4b3accf9c4c859_1440w.jpg)

PPU的实现：

![](https://pic1.zhimg.com/v2-9d752127c0367594a9ed0210b24b7264_1440w.jpg)

电压域划分举例：

![](https://picx.zhimg.com/v2-99323bfefb1e37b1e949713ff22b24b7_1440w.jpg)

## 2.2.2 imx8q实现

![](https://pic1.zhimg.com/v2-3ae126b0426605d9f60a6ef544705202_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-c9ad5a691fb6a97ca31c32e1e581cbf2_1440w.jpg)

![](https://picx.zhimg.com/v2-285b0ef4565880cd7096167cec4e7467_1440w.jpg)

电源模式：

![](https://picx.zhimg.com/v2-c59274876dd1ec4eecff653097de582f_1440w.jpg)

clock：

![](https://picx.zhimg.com/v2-d923f64e6de42097f1072cc9e26afab7_1440w.jpg)

RESET：

![](https://pica.zhimg.com/v2-625f9e6d93835f7955d064791035a298_1440w.jpg)

唤醒源：

![](https://picx.zhimg.com/v2-21cd7ee341484d73987858cf3e53bec7_1440w.jpg)

内存访问：

![](https://pic1.zhimg.com/v2-a0cf68a8b8221618ca029f84d250db78_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-ef79e1c2e2263ac5fde5c08f1f74adf8_1440w.jpg)
...
但是当你一直沿着软件 **OS-》ATF** 往下分析发现还有这个 **幕后黑手** SCP。

可以在ARM官网下载PCSA规范文档： [developer.arm.com/docum](https://link.zhihu.com/?target=https%3A//developer.arm.com/documentation/den0050/d/%3Flang%3Den)
...
![](https://pic2.zhimg.com/v2-5f882413f06df148f57c80b9836e4199_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-5f07eb89cd7590a97e62edc59b8cbe69_1440w.jpg)

软件分层：

![](https://picx.zhimg.com/v2-c1578eca65679f376521bfedd76e0c6f_1440w.jpg)

bind：

![](https://pic2.zhimg.com/v2-d1993ea91861e0ce262bf3ed7581913b_1440w.jpg)

## 3.3 ARM SCP入门-AP与SCP通信

![](https://pic3.zhimg.com/v2-3e3d75eaee4dc8ca2bd9b79a60f8d6ec_1440w.jpg)

电源管理相关软件协议栈：

![](https://pica.zhimg.com/v2-48d03916b1bb28d84499b278fb8e0bde_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-0028373dedcb8fbd22018c884aeeaf61_1440w.jpg)
...
- IST及STL测试程序的集成
- 对于供电NPU耗电多且要求文档需要独立的外部电压芯片提供电压电流，AON区域也需要独立的供电，另外DDR也耗电多。
- 测试功耗需要去供电电源芯片里面拿到电压电流，或者SoC内部的PVT里可以监测电压温度等信息。

> 后记：  
> 又是一篇概念很大的文章，其中一段话就能独立展开写一篇。感兴趣的可以留言，可以独立展开一篇。
