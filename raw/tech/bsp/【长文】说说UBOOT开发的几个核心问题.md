...
source: "https://zhuanlan.zhihu.com/p/109194211"
author:
  - "[[韦东山嵌入式关注公众号: 百问科技，学习更多嵌入式干货]]"
published:
created: 2026-05-02
description: "作者：李壮 来源： http://www.elecfans.com/d/617674.html 本文目的，有助于嵌入式新手对uboot有个大概的了解，方便老手回忆，复习。 为什么需要U-BOOT? 1、计算机系统的主要部件 （1）计算机系统就是以CPU为核心…"
tags:
  - "clippings"
---
...
来源： [elecfans.com/d/617674.h](https://link.zhihu.com/?target=http%3A//www.elecfans.com/d/617674.html)
...
　CPU+外部存储器（Flash/硬盘）+ 内部存储器（ [DDR SDRAM](https://zhida.zhihu.com/search?content_id=112395910&content_type=Article&match_order=1&q=DDR+SDRAM&zhida_source=entity) /SDRAM/SRAM）

**2、PC机的启动过程**

（1）部署：

　　典型的PC机的 [BIOS](https://zhida.zhihu.com/search?content_id=112395910&content_type=Article&match_order=1&q=BIOS&zhida_source=entity) 程序部署在PC机主板上（随主板出厂时已经预制了），操作系统部署在硬盘上，内存在掉电时无作用，CPU在掉电时不工作。

（2）启动过程：

　　PC上电后先执行BIOS程序（实际上PC的BIOS就是 [NorFlash](https://zhida.zhihu.com/search?content_id=112395910&content_type=Article&match_order=1&q=NorFlash&zhida_source=entity) ），BIOS程序负责初始化DDR内存，负责初始化硬盘，然后从硬盘上将OS镜像读取到DDR中，然后跳转到DDR中去执行OS直到启动（OS启动后BIOS就无用了）
...
**1、从裸机程序镜像 [uboot.bin](https://zhida.zhihu.com/search?content_id=112395910&content_type=Article&match_order=1&q=uboot.bin&zhida_source=entity) 说起**
...
譬如说我们 [tftp](https://zhida.zhihu.com/search?content_id=112395910&content_type=Article&match_order=1&q=tftp&zhida_source=entity) 0x23E00000 zImage去下载zImage到内存的0x23E00000处就会出错，因为这个内存处实际是uboot的镜像所在。这样下载会导致下载的zImage把内存中的uboot覆盖掉。
...
[wiki.100ask.org/VMwareA](https://link.zhihu.com/?target=http%3A//wiki.100ask.org/VMwareAndUbuntuNetworkSetupGuide)
...
（6）然后在开发板的uboot下先ping通虚拟机ubuntu，然后再尝试下载：tftp 0x30000000 zImage-qt（意思是将服务器上名为zImage-qt的文件下载到开发板内存的0x30000000地址处。），下载好再擦除。烧写。
...
注意，这里的0x30000000也可以直接写作30000000，意思是一样的（uboot的命令行中所有数字都被默认当作十六进制处理，不管你加不加0x都一样）。
...
**4、启动内核指令： [bootm](https://zhida.zhihu.com/search?content_id=112395910&content_type=Article&match_order=1&q=bootm&zhida_source=entity) 、 [go](https://zhida.zhihu.com/search?content_id=112395910&content_type=Article&match_order=1&q=go&zhida_source=entity)**
...
对uboot有任何疑问都可以加入韦东山微信粉丝群进行讨论，购买产品的加售后群。组队学习，更快成长。

加群方式：

添加管理员微信： **13163769879** ，备注：嵌入式。

[所属专栏 · 2025-12-25 17:53 更新](https://zhuanlan.zhihu.com/c_118891916)

[![](https://pica.zhimg.com/v2-ede5849e82d5d2ac99d74873da4371d3_720w.jpg?source=172ae18b)](https://zhuanlan.zhihu.com/c_118891916)
...
发布于 2020-02-26 10:50[想投身数字IC设计方向，硕士阶段应该怎么学习？](https://www.zhihu.com/question/21501593/answer/2273565904)
