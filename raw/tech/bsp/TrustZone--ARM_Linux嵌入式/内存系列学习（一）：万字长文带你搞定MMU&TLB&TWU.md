...
source: "https://zhuanlan.zhihu.com/p/650956048"
author:
  - "[[TrustZone​​​海思技术有限公司 员工]]"
published:
created: 2026-05-03
description: "大家好！我是不知名的安全工程师Hkcoco！ 欢迎大家关注我的微信公众号：TrustZone | CSDN：Hkcoco 获取更多精彩内容哦！！！ 前言一直在学习内存管理，也知道MMU是管理内存的映射的逻辑IP，还知道里面有个TLB。 今…"
tags:
  - "clippings"
---
...
欢迎大家关注我的微信公众号： [TrustZone](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=TrustZone&zhida_source=entity) | CSDN：Hkcoco
...
一直在学习内存管理，也知道 [MMU](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=MMU&zhida_source=entity) 是管理内存的映射的逻辑IP，还知道里面有个 [TLB](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=TLB&zhida_source=entity) 。

![](https://picx.zhimg.com/v2-948a10712a1d51277ba68ff633b3fb93_1440w.png)
...
**MMU（Memory Management Unit，内存管理单元）是一种硬件模块** ，用于在 **CPU和内存之间实现 [虚拟内存](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98&zhida_source=entity) 管理** 。

**其主要功能是将虚拟地址转换为物理地址** ，同时提供访问权限的控制和缓存管理等功能。

放在整个大系统多核架构里面，每个处理器内置了MMU模块，MMU模块包含了TLB和 [TWU](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=TWU&zhida_source=entity) 两个子模块。

![](https://pic1.zhimg.com/v2-60f5d61b20cd533c8ee854d10473cbf4_1440w.png)

在这里插入图片描述

## 1-宏观理解

地址空间是一个抽象的概念，由CPU体系架构的地址总线决定，一般等同于CPU的寻址范围、x位处理器中的x。 **地址空间一般分为 [虚拟地址空间](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%9C%B0%E5%9D%80%E7%A9%BA%E9%97%B4&zhida_source=entity) 和 [物理地址空间](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%9C%B0%E5%9D%80%E7%A9%BA%E9%97%B4&zhida_source=entity) 。**
...
![](https://pic3.zhimg.com/v2-b3d969bbea3197c6a719d61027aede78_1440w.png)

在这里插入图片描述

## 2-微观理解

内存管理单元（MMU）的一个重要功能是 **使系统能够运行多个任务，作为独立的程序运行在他们自己的**  
**私有虚拟内存空间。**
...
![](https://pic2.zhimg.com/v2-5cd198e7b2f7797936aebf9ccc98d98b_1440w.png)
...
![](https://pic2.zhimg.com/v2-d029cdb750ffff6a5f7fa7d3d0b85cc9_1440w.png)
...
![](https://pic4.zhimg.com/v2-febe06ac809c68f76d42a8d9105ed9cf_1440w.png)
...
CPU发出的虚拟地址由两部分组成：VPN和offset，VPN（virtual page number）是 [页表](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=%E9%A1%B5%E8%A1%A8&zhida_source=entity) 中的条目number，而offset是指页内偏移。

最终转换后的物理地址也有两部分：PFN和offset，PFN（ Physical frame number）是物理页框number，offset和上面虚拟地址的offset相同，是页内偏移。

![](https://pic3.zhimg.com/v2-fffdb878d1368fe460cd9010935b619e_1440w.png)
...
![](https://pic1.zhimg.com/v2-504e3197d9699416eae4f407704d210e_1440w.png)
...
![](https://pic4.zhimg.com/v2-f9a369c8e626dc584da3265296d31c23_1440w.png)

在这里插入图片描述

在EL2系统中，MMU地址转换时，会自动使用TTBR2\_EL1指向的页表。

在EL3系统中，MMU地址转换时，会自动使用TTBR3\_EL1指向的页表。

![](https://pic4.zhimg.com/v2-1f049c416c132303ebb23ae11f5843b1_1440w.png)

在这里插入图片描述

### 4-启用hypervisor

如果启用了hypervisor那么虚拟地址转换的过程将有VA—>PA变成了VA—>IPA—>PA, 也就是要经过两次转换.在guestos(如linux kernel)中转换的物理地址，其实不是真实的物理地址(假物理地址)，然后在EL2通过VTTBR0\_EL2基地址的页表转换后的物理地址，才是真实的硬件地址。

![](https://pic4.zhimg.com/v2-b58ea9ee4bd23fb8cb44c99c46d0605d_1440w.png)
...
![](https://pic3.zhimg.com/v2-bd42b4d5f544a3b875710f70a69dac24_1440w.png)

在这里插入图片描述

![](https://pic4.zhimg.com/v2-057190357df6d1a94b79a7e04bbcc699_1440w.png)

在这里插入图片描述

![](https://pic3.zhimg.com/v2-7f4063cf440eaf28e162bdcaef2f4fa6_1440w.png)
...
![](https://pica.zhimg.com/v2-1ccf54a9b2ec573868430d68c0cbf6e6_1440w.png)

在这里插入图片描述

TLB 是 translation lookaside buffer 的简称。首先，我们知道 MMU 的作用是把虚拟地址转换成物理地址。

![](https://picx.zhimg.com/v2-e3ead541ddb94b4708585fcb32581c71_1440w.png)
...
![](https://pic3.zhimg.com/v2-28354d3781a2aa94efd0326bd671d8b8_1440w.png)
...
![](https://pic1.zhimg.com/v2-803d59641d87a2eae56802cb6c8c08a0_1440w.png)
...
![](https://pica.zhimg.com/v2-903e62d7adbe7dabddeed1f5b7ee48e0_1440w.png)
...
![](https://pica.zhimg.com/v2-dafe029e3790bd177f2fc23bda8243ac_1440w.png)
...
- [虚拟页号](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E9%A1%B5%E5%8F%B7&zhida_source=entity) （Virtual Page Number,VPN）：用于定位页表的PTE。
...
- [物理页号](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E9%A1%B5%E5%8F%B7&zhida_source=entity) （Physical Page Number,PPN）：定位物理内存中块的位置。
...
![](https://pica.zhimg.com/v2-9523694907be17187300d08331b0c82c_1440w.png)
...
![](https://pic2.zhimg.com/v2-d72a753f964bccd244c60f89ce5b2969_1440w.png)
...
- b) MMU的TLB没有命中，通过TWU遍历主存页表中的PTEA（PTE地址）。
- c) 主存向MMU返回PTE。
- d) PTE中有效位是0，MMU触发一次异常，CPU相应 [缺页异常](https://zhida.zhihu.com/search?content_id=232786360&content_type=Article&match_order=1&q=%E7%BC%BA%E9%A1%B5%E5%BC%82%E5%B8%B8&zhida_source=entity) ，运行相应的处理程序。
...
![](https://picx.zhimg.com/v2-a9c37c9d59ffe2bfc4dd7cea5a69fe8d_1440w.png)
...
转换控制寄存器（Translation Control Register）TCR\_EL1的字段T0SZ定义了必须是全0的最高位的数量，字段T1SZ定义了必须是全1的最高位的数量，用户虚拟地址的宽度是（64-TCR\_EL1.T0SZ），内核虚拟地址的宽度是（64-TCR\_EL1.T1SZ）。 （全是0是代表的用户，全是1代表的是内核。）
...
- [jianshu.com/p/ef1e93e9d](https://link.zhihu.com/?target=https%3A//www.jianshu.com/p/ef1e93e9d65b)
- [zhuanlan.zhihu.com/p/57](https://zhuanlan.zhihu.com/p/570452487)
- [zhuanlan.zhihu.com/p/48](https://zhuanlan.zhihu.com/p/487386274)
- [zhuanlan.zhihu.com/p/59](https://zhuanlan.zhihu.com/p/596039345)
- [jianshu.com/p/e4bc4a8e2](https://link.zhihu.com/?target=https%3A//www.jianshu.com/p/e4bc4a8e2553)
- [taodudu.cc/news/show-52](https://link.zhihu.com/?target=http%3A//www.taodudu.cc/news/show-523335.html%3Faction%3DonClick)
- [mp.weixin.qq.com/s?](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg3ODU3Nzk3MQ%3D%3D%26mid%3D2247502406%26idx%3D1%26sn%3D0287f11c54654968abe656280ffac0c5%26scene%3D21%23wechat_redirect)
- [zhuanlan.zhihu.com/p/65](https://zhuanlan.zhihu.com/p/65348145)
- [zhuanlan.zhihu.com/p/43](https://zhuanlan.zhihu.com/p/436719684?utm_source=zhihu&utm_id=0)
- 《嵌入式系统开发指南》
- 《深度探索嵌入式系统开发》
- 《计算机架构与体系》
...
编辑于 2023-08-22 00:49・四川[进销存ERP、进销存库存管理系统开发案例](https://zhuanlan.zhihu.com/p/658475128)
