...
从主机的角度来看，与DIMM上不同的颗粒的距离是不同的。而从DIMM的角度来看，时钟（黑色）与数据（绿色）之间的相对延迟对于不同颗粒是不同的。训练的目的即消除这两个不同对数据读写的影响，确保高速内存系统在各种条件下都能稳定、可靠运行的重要技术手段。
...
4. **DLL(Delay Lock Loop)**: 目的...

（原文有配图说明各种training波形和时序，此处保留核心文字内容）
