...
for (const file of files) {
  ...
  if (!sourceContent.includes('domain: tech/bsp')) continue;
  ...
  if (!fs.existsSync(rawFullPath)) {
    failed++;
    console.log('  ✗', file, '- raw not found:', rawRelPath);
    continue;
  }
  ...
  // Strategy 3: Find important sentences with key patterns
  if (quotes.length < 3) {
...
console.log('Total BSP files:', processed + skipped + failed);
