...
本文介绍了 ARM SOC 的启动流程，以芯片内部各核心组件的 Firmware 加载和启动为线索。ARM SOC 通常包含 ARM 处理器核心（Cortex-A/R/M）、GPU、内存控制器（DDRC）、外设接口、多媒体处理单元（VPU/JPU/DPU）、网络接口、电源管理单元（PMU）和安全模块（TEE/TrustZone）。启动过程中，专门的 BOOT Core（如 ARM7 或 Cortex-M3）负责系统初始化，包括初始化外设接口、加载固件到 SRAM、配置电源和时钟、初始化 DDR，最后解除主处理器（AP CPU）的复位状态。
...
- 芯片上电后立即运行固化启动代码
- 初始化基本外设接口（SPI/SDIO）
- 从外部存储（Flash/SD 卡）读取固件到 SRAM
- 完成电源管理、时钟配置、片内总线初始化
- 初始化 DDR 控制器（DDRC）
- 加载 PM FW 和 BL2 FW 到 DDR
- 解除 AP CPU 复位，启动高性能核心
