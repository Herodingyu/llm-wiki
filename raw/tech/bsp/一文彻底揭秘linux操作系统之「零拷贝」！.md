...
source: "https://zhuanlan.zhihu.com/p/430848775"
author:
  - "[[linux]]"
published:
created: 2026-05-02
description: "- 前言 - 零拷贝（Zero-Copy）是一个大家耳熟能详的概念，那么，具体有哪些框架会使用到零拷贝呢？在思考这个问题之前，让我们先一起探寻一下零拷贝机制的底层原理。推荐视频： 手写用户态协议栈以及零拷贝的实现 …"
tags:
  - "clippings"
---
[收录于 · linux服务器开发](https://www.zhihu.com/column/c_1242422263888064512)
...
[手写用户态协议栈以及零拷贝的实现](https://link.zhihu.com/?target=https%3A//www.bilibili.com/video/BV1if4y1j7p3/)

[为什么不推荐做mcu与qt开发，c++ linux后台服务器开发很香吗？](https://link.zhihu.com/?target=https%3A//www.bilibili.com/video/BV1Rv41137jY/)
...
（1） [CPU COPY](https://zhida.zhihu.com/search?content_id=183873062&content_type=Article&match_order=1&q=CPU+COPY&zhida_source=entity)
...
（2） [DMA COPY](https://zhida.zhihu.com/search?content_id=183873062&content_type=Article&match_order=1&q=DMA+COPY&zhida_source=entity)
...
内存管理系统和 [VFS](https://zhida.zhihu.com/search?content_id=183873062&content_type=Article&match_order=1&q=VFS&zhida_source=entity) 只与 Page Cache 交互，内存管理系统负责维护每项 Page Cache 的分配和回收，同时在使用 memory map 方式访问时负责建立映射。
...
![](https://pic1.zhimg.com/v2-5a58db1de950db53a6c66c37bcf6e25c_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-6e70a0c9e37a3dd908b694578fc1a8f5_1440w.jpg)

【文章福利】需要C/C++ Linux服务器架构师学习资料加群 [812855908](https://link.zhihu.com/?target=https%3A//jq.qq.com/%3F_wv%3D1027%26k%3DckYqybaE) （资料包括C/C++，Linux，golang技术，内核，Nginx，ZeroMQ，MySQL，Redis，fastdfs，MongoDB，ZK，流媒体，CDN，P2P，K8S，Docker，TCP/IP，协程，DPDK，ffmpeg等）

![](https://pic2.zhimg.com/v2-016587b1e95f5c6148709dbac1fa8d7b_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-4339abe8f90eaf8ccc294907283a43a2_1440w.jpg)
...
ssize\_t senfile(int out\_fd,int in\_fd,off\_t\* offset,size\_t count);

1. out\_fd 是写出的文件描述符,而且必须是一个 socket；
2. in\_fd 是读取内容的文件描述符,必须是一个真实的文件, 不能是管道或 socket；
3. offset 是开始读的位置；
4. count 是将要读取的字节数。
...
![](https://pic2.zhimg.com/v2-c5b255c1c42e195f32c4afc3cb82a26b_1440w.jpg)
...
支持 [scatter-gather](https://zhida.zhihu.com/search?content_id=183873062&content_type=Article&match_order=1&q=scatter-gather&zhida_source=entity) 特性的 sendFile 的 IO 流程：
...
![](https://pica.zhimg.com/v2-b72230f321d142066a2bad52c8e0f3c4_1440w.jpg)
