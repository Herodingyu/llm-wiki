---
title: "超详细【Uboot驱动开发】（三）Uboot驱动模型"
source: "https://zhuanlan.zhihu.com/p/460754843"
author:
  - "[[嵌入式艺术​计算机技术与软件专业技术资格证持证人]]"
published:
created: 2026-05-02
description: "三、Uboot驱动模型 全文耗时一周，精心汇总20000余字，希望对大家有所帮助，感觉可以的点赞，关注，不迷路，后续还有更多干货！ 看文章前，答应我，静下心来，慢慢品！3.1、什么是Uboot驱动模型学过Linux的朋友基…"
tags:
  - "clippings"
---
[收录于 · 超详细【Uboot驱动开发】](https://www.zhihu.com/column/c_1468303841838444544)

186 人赞同了该文章

## 三、Uboot驱动模型

![](https://pic1.zhimg.com/v2-3f4caa722a78951f624572edc90f1e54_1440w.jpg)
...
![](https://picx.zhimg.com/v2-d6864eec20d78302baa063d43f011867_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-66eba87a4b096b80ccea9d35c376166b_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-477629e30bc02dab0504f4aec6175a16_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-2ae3e7d1f83c0fa5ff1132931e81545e_1440w.jpg)
...
![](https://pic1.zhimg.com/v2-6aa397da643e136a33ba4b31651edd62_1440w.jpg)
...
- **uclass\_driver：** 设备组公共行为对象， `uclass` 的驱动程序，主要将 `uclass` 管理的设备和驱动实现绑定、注册，移除等操作。

通过这两个结构体的引入，可以将毫不相关的 `udevice` 是 `driver` 关联起来！

`udevice` 与 `driver` 的绑定：通过驱动的 `of_match` 和 `compatible` 属性来配对，绑定。

![](https://pic2.zhimg.com/v2-e0d3b18befede27394bbd17022c49ff9_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-e6838a6a49e36ac8c424f83573cceded_1440w.jpg)

![](https://pic1.zhimg.com/v2-2b7e20de19a54f5403e42a9bea4829f2_1440w.jpg)
...
- 当dm\_init\_and\_scan的参数为 `true` 时，只会对带有“ `u-boot,dm-pre-reloc` ”属性的节点进行解析。而当参数为 `false` 的时候，则会对所有节点都进行解析。

**DM初始化的大体步骤如下：**

![](https://pica.zhimg.com/v2-dcf449348d4b77fba0996e385838fd4c_1440w.jpg)
...
![](https://pic1.zhimg.com/v2-13bbe1e45a6ee30e546e1dd3c99e2b0c_1440w.jpg)

详细代码在这里就不展开来叙述了！

在MMC驱动初始化后，有没有注意到 `mmc_probe` 这个函数，该函数就是间接调用了我们驱动编写的 `probe` 函数。

执行流程在上面已经很清楚了：根据 `uclass_id` ，调用 `` `uclass_get_device_by_seq `` 来得到 `udevice` ，进而调用 `device_probe` 来找到对应驱动的 `probe` 。
...
## 3.8、DM模型——uclass与uclass\_driver绑定
...
[超详细【Uboot--MMC驱动开发】（三）Uboot驱动模型\_Donge的博客-CSDN博客](https://link.zhihu.com/?target=https%3A//blog.csdn.net/dong__ge/article/details/122637220)

编辑于 2023-12-21 08:44・IP 属地未知[AI项目拆解：大厂RAG知识库智能问答系统](https://zhuanlan.zhihu.com/p/1985739871751538565)

[

今天给大家拆解一下基于 RAG 构建的电商知识库智能问答系统。 \[图片\] 一、什么是 RAG?RAG 全称 Retrieval-Augmented Generati...

](https://zhuanlan.zhihu.com/p/1985739871751538565)