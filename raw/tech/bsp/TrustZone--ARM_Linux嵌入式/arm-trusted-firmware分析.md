...
source: "https://zhuanlan.zhihu.com/p/722187249"
author:
  - "[[TrustZone​​​海思技术有限公司 员工]]"
published:
created: 2026-05-03
description: "概要ARM Trusted Firmware实现了一个可信引导过程，并给操作系统提供运行环境（SMC调用服务）。本文分析基于ARM Trusted Firmware - version 1.3，代码托管与 GitHub。引导过程分为多步，每一步为一个独立的程序。…"
tags:
  - "clippings"
---
...
[ARM Trusted Firmware](https://zhida.zhihu.com/search?content_id=248607952&content_type=Article&match_order=1&q=ARM+Trusted+Firmware&zhida_source=entity) 实现了一个可信引导过程，并给操作系统提供运行环境（ [SMC调用服务](https://zhida.zhihu.com/search?content_id=248607952&content_type=Article&match_order=1&q=SMC%E8%B0%83%E7%94%A8%E6%9C%8D%E5%8A%A1&zhida_source=entity) ）。本文分析基于ARM Trusted Firmware - version 1.3，代码托管与 **[GitHub](https://link.zhihu.com/?target=https%3A//github.com/ARM-software/arm-trusted-firmware)** 。
...
此模块通过操作crypto\_lib\_desc变量，实现模块初始化、校验签名、校验哈希。函数声明如下：
...
auth\_mod实现了一个校验镜像的模型，此模型通过结构体 `auth_img_desc_t` 描述。
...
### io\_dev\_connector\_t

io\_dev\_connector\_t对应一个驱动
...
系统通过 **dev\_count** 、 **devices** ，来维护一个注册的设备列表
...
系统通过 **entity\_pool** 、 **entity\_map** 、 **entity\_count** 来维护已经打开的文件
...
EL3初始化通过宏 **el3\_entrypoint\_common** 实现，此宏位于 `include/common/aarch64/el3_common_macros.S` 中
...
此部分与具体硬件相关，需要具体平台实现 **void platform\_mem\_init(void)**
...
使用SP\_EL0作为C语言的堆栈指针，此处有单内核堆栈初始化与多内核堆栈初始化，这里只关注多内核堆栈初始化。这样每个内核会获取到一个不同的堆栈指针。
...
每个cpu有一个 **cpu\_data\_t** 的数据，定义在 `lib/el3_runtime/aarch64/cpu_data_array.c` 中

```
cpu_data_t percpu_data[PLATFORM_CORE_COUNT];
```

为了便于cpu访问到自己的数据通过 **TPIDR\_EL3** 指向自己的 **cpu\_data\_t** ，初始化过程如下
...
通过读取 **TPIDR\_EL3** 获取 **cpu\_data\_t**
...
**DEFINE\_REG\_STRUCT** 用于定义一组寄存器， **name** 寄存器组的名字， **num\_regs** 为寄存器的个数
...
中断现场保护通过 **SP\_EL3** 完成，但每个处理器需要分别初始化自己的 **SP\_EL3** 。这部分在异常退出之前（引导下一级代码之前）通过设置 **SP\_EL3** 完成，这样每个处理器发生中断时就有了独立的 **上下文（context）** ，具体通过 **cm\_set\_next\_context** 函数实现
...
镜像加载通过 **load\_auth\_image** 函数实现，并实现对镜像的认证
...
具体实现通过 **load\_auth\_image\_internal** 实现
...
**auth\_mod\_verify\_img** 根据镜像描述符描述的认证方式对镜像进行校验认证
...
通过 **DECLARE\_RT\_SVC** 的宏声明一个SMC服务，服务被放在 **rt\_svc\_descs** 段中，并在链接脚本中指定 **rt\_svc\_descs** 的起始结束符号（ **\_\_RT\_SVC\_DESCS\_START\_\_** / **\_\_RT\_SVC\_DESCS\_END\_\_** ）
...
每个服务需要初始化（ **rt\_svc\_desc\_t.init** ），而且链接后所有的服务形成一个数组被放在\*\*\_\_RT\_SVC\_DESCS\_START\_\_ **和** \_\_RT\_SVC\_DESCS\_END\_\_ **符号之间，需要建立一个索引通过服务号找到对应的** rt\_svc\_desc\_t\*\*
...
通过初始化构建的索引找到对应的服务，执行 **rt\_svc\_desc\_t.handle**
...
以上代码是一个简化的主函数。在SMC服务初始化时，为TOS服务的SMC服务将根据 **TSP\_INIT\_ASYNC** 调用 **bl31\_set\_next\_image\_type** 、 **bl31\_register\_bl32\_init**

**bl31\_set\_next\_image\_type** 相关部分， **TSP\_INIT\_ASYNC** 未定义时此不分发挥作用
...
**bl31\_register\_bl32\_init** 相关部分， **TSP\_INIT\_ASYNC** 定义时此不分发挥作用
...
上面遗产处理中，有一组特别的函数 **tspd\_synchronous\_sp\_entry** 、 **tspd\_synchronous\_sp\_exit**

**tspd\_synchronous\_sp\_entry** 用于退出异常执行下一级BL

**tspd\_synchronous\_sp\_exit** 用于下一级BL执行结束后通过SMC返回，然后跳转到 **tspd\_synchronous\_sp\_entry** 后继续执行。
...
**tspd\_enter\_sp** 、 **tspd\_exit\_sp** 实现细节如下
...
handle把SP\_EL3传递给函数，便于修改返回值（现场保护通过SP\_EL3完才）
...
BL1使用SP\_EL0作为其C程序运行的堆栈，在进入异常时默认认为SPSEL=1，使用的是SP\_EL3。这样，保存的线程在SP\_EL3中。切换到SP\_EL0执行程序，并通过SP\_EL3来修改堆栈。
...
bl2\_main为bl2的主程序，位于bl2/bl2\_main.c中
...
发布于 2024-09-26 00:04・四川[ARM](https://www.zhihu.com/topic/19553303)[怎么复习信息系统项目管理师？](https://www.zhihu.com/question/285411826/answer/127590858695)

[

“软考高项通关速览：从零到拿证的实战地图”这篇攻略为你划清备考迷雾：考试结构：综合知识：75道选择题（项目管理+时...

](https://www.zhihu.com/question/285411826/answer/127590858695)